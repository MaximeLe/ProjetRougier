...
<tiles:putAttribute name="microformat">itemscope itemtype="http://schema.org/ItemPage"</tiles:putAttribute>
				...
				<div class="infos">
					...
					<div itemprop="offers" itemscope itemtype="http://schema.org/Offer">
						...
						<c:choose>
							...
							<c:otherwise>
								<div class="stock mp" itemprop="availability" href="http://schema.org/InStock">
						...
						<div class="quantity" style="clear:both">
							<div class="select">
								<input type="hidden" id ="quantityDiscriminantStock" value="" onChange="setQuantityDiscriminant();"/>
								<img onClick="decQuantityDiscriminant(${ceDisplay.catalogEntry.catalogEntryId});" src="${contextPath}/resources/${codeIsoLang}/img/merchandising/btn_moins.png" width="15" height="15" alt="" border="0"/>
								<input type="text" id="quantityDiscriminant" onChange="setQuantityDiscriminant();" value="1" maxlength="4">
	...
	<tiles:putAttribute name="moreJS">
		...
		var currency = '${devise}';
			...
			var entryId = '${ceDisplay.catalogEntryPrice.catalogEntryId}';
			var skuId;
			
			var tabMainPicturePath = new Array();
// 			console.log("catalogEntryDiscriminant: ");
...
// 				console.log(tabMainPicturePath[0]);
			...
			$(document).ready(
				function (){
					...
					callDataForAjaxDiscriminant('previousNeighbor', 'nextNeighbor', '${urlRewrite}', '${ceDisplay.catalogEntry.catalogEntryId}', '${catalogId}', context);
					Shadowbox.init();
				}
			);
		</script>
		...
		<c:if test="${stockMagasinActivated}">
			...
			<script type="text/javascript">
				...
				var listeCodeProduit = $.parseJSON('${listeCodeProduit}');
				
				// On active la modification du discriminant dans la liste
				function modifierDiscriminantDisponibiliteMagasin() {
					// A l'initialisation de la page
					var indexSelectionne = $("#selectDiscriminant option:selected").prevAll().size();
					showDisponibiliteMagasinDiscriminant($('#selectDiscriminant').val(),listeCodeProduit,indexSelectionne);
					
					$('#selectDiscriminant').change(function() {
						showDisponibiliteMagasinDiscriminant($('#selectDiscriminant').val(),listeCodeProduit,this.selectedIndex);
					});
				}
			</script>
